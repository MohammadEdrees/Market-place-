...
// N-tier wiring: Application owns the services the controllers delegate to; Infrastructure owns
// EF Core on SQL Server (connection string "ConnectionStrings:MarketDb" in appsettings.json; the
// schema is created by the migrations under Infrastructure/Migrations — DbInitializer calls
// Database.Migrate() at startup, then seeds empty tables).
...
// JWT minting lives in this tier; the Application layer consumes the interface only.
